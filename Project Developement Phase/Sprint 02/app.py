...
import copy
import calendar
import tracker_dashboard
import tracker_expenses
import tracker_budgets
import tracker_categories
import tracker_reports
import tracker_account
...
def index():
    ...
    if request.method == "GET":
        ...
        categories = tracker_categories.getSpendCategories(session["user_id"])

        # Get the users payers (for quick expense modal)
        payers = tracker_account.getPayers(session["user_id"])

        # Get todays date (for quick expense modal)
        date = datetime.today().strftime('%Y-%m-%d')

        # Get the users income
        income = tracker_account.getIncome(session["user_id"])

        # Get current years total expenses for the user
        expenses_year = tracker_dashboard.getTotalSpend_Year(session["user_id"])

        # Get current months total expenses for the user
        expenses_month = tracker_dashboard.getTotalSpend_Month(
            session["user_id"])

        # Get current week total expenses for the user
        expenses_week = tracker_dashboard.getTotalSpend_Week(session["user_id"])

        # Get last 5 expenses for the user
        expenses_last5 = tracker_dashboard.getLastFiveExpenses(
            session["user_id"])

        # Get every budgets spent/remaining for the user
        budgets = tracker_dashboard.getBudgets(session["user_id"])

        # Get weekly spending for the user 
        weeks = tracker_dashboard.getLastFourWeekNames()
        spending_week = tracker_dashboard.getWeeklySpending(
            weeks, session["user_id"])

        # Get monthly spending for the user (for the current year)
        spending_month = tracker_dashboard.getMonthlySpending(
            session["user_id"]) 
